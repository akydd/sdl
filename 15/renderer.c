...
SDL_RendererFlip int_to_rendererflip(int);
...
void render_graphic(renderer *renderer, world *world, cache *cache, int id) {
	...
	if((mask & ROTATION) == ROTATION) {
		...
		SDL_RendererFlip flip = int_to_rendererflip(world->rotations[id].flip);
		SDL_RenderCopyEx(renderer->sdl_renderer, texture, source_ptr, size_ptr, world->rotations[id].angle, &center, flip);
	} else {
		SDL_RenderCopy(renderer->sdl_renderer, texture, source_ptr, size_ptr);
	}
}

SDL_RendererFlip int_to_rendererflip(int x) {
