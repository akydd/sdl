...
void handle_key_input(SDL_Event e, world *world)
{
    int new_entity;
...
void handle_mouse_click_input(SDL_Event e, world *world)
{
    int new_entity, x, y;
    world_delete_all_entities(world);
    new_entity = world_create_entity(world);
    ...
    world_add_position_component(world, new_entity, x, y);
}

void input_system_update(SDL_Event e, world *world) {
    if(e.type == SDL_KEYDOWN) {
        handle_key_input(e, world);
    } 

    if(e.type == SDL_MOUSEBUTTONDOWN) {
        handle_mouse_click_input(e, world);
    }
}
